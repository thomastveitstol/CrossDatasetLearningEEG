"""
Script for re-running experiments that failed when re-running
"""
import copy
import os
import time
...
def _suggest_run(results_dir) -> str:
    # Get the list of experiments to re-run
    path = Path(os.path.dirname(__file__)) / "rerun_again.csv"
    affected_runs = tuple(pandas.read_csv(path)["rerun_again"])

    # Get the list of experiments which has already been re-re-run
