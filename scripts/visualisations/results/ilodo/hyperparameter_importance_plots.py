...
from cdl_eeg.data.analysis.results_analysis import get_config_file, SkipFold, higher_is_better, \
    PRETTY_NAME, get_all_ilodo_runs, get_ilodo_val_dataset_name
...
def _get_test_performance(path,  *, target_metric, selection_metric, datasets):
...
def create_studies(*, datasets, runs, direction, results_dir, target_metric, selection_metric,
                   dist_config, hyperparameters=None):
    """
...
def main():
    ...
    runs = get_all_ilodo_runs(results_dir=results_dir, successful_only=True)  # [:40]
    ...
    studies = create_studies(
        ...
        selection_metric=selection_metric, hyperparameters=hyperparameters, dist_config=config_dist
    )
