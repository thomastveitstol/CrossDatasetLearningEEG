...
def main_inverted_lodo():
    ...
    if hyperparam.variable_type == VariableType.NUMERICAL:
        ...
        data: Dict[str, List[Any]] = {hyperparameter_name: [], f"Performance ({PRETTY_NAME[performance_metric]})": [],
                                      "Target dataset": []}
        for dataset_name, test_performance in test_performances.items():
            for performance, hyperparameter_value in zip(test_performance, hyperparameter):
                ...
                data[f"Performance ({PRETTY_NAME[performance_metric]})"].append(performance)
                data["Target dataset"].append(PRETTY_NAME[dataset_name])

        ax = seaborn.boxplot(data, x=f"Performance ({PRETTY_NAME[performance_metric]})", y="Target dataset",
                             hue=hyperparameter_name)
